...
LOBBY_CHANNEL_NAME = "도서관-로비"
DUEL_CHANNEL_NAME = "언어의-층-결투장"
...
# 채널 판별
# =========================
def is_lobby_channel(channel):
    return hasattr(channel, "name") and channel.name == LOBBY_CHANNEL_NAME


def is_duel_channel(channel):
    return hasattr(channel, "name") and channel.name == DUEL_CHANNEL_NAME


def can_use_bot(channel):
    return is_lobby_channel(channel) or is_duel_channel(channel)
...
def increment_daily_progress(user_id: int, progress_type: str, amount: int = 1):
    ...
    uid = str(user_id)
    daily_progress.setdefault(uid, {})
    current = daily_progress[uid].get(progress_type, 0)
    daily_progress[uid][progress_type] = current + amount

    if quest["type"] == progress_type and daily_progress[uid][progress_type] >= quest["goal"]:
        return complete_daily(user_id)

    return None


async def send_daily_reward_message(channel, member, result):
    if not result:
        return
...
async def run_pvp_duel(ctx, challenger, target):
    ...
    for member in [challenger, target]:
        ...
        if result:
            await send_daily_reward_message(ctx.channel, member, result)

    result = increment_daily_progress(winner.id, "duel_win", 0)
    if result:
        await send_daily_reward_message(ctx.channel, winner, result)
...
async def run_boss_duel(ctx, user):
    ...
    if user_hp > 0:
        ...
        if result:
            await send_daily_reward_message(ctx.channel, user, result)
    else:
...
async def on_message(message):
    ...
    if not can_use_bot(message.channel):
        return
    ...
    if message.author.bot:
        if HORDE_BOT_ID != 0 and message.author.id == HORDE_BOT_ID:
            if is_lobby_channel(message.channel) and random.random() < 0.18:
    ...
    # 로비 대화
    if is_lobby_channel(message.channel):
        if "호드" in content and random.random() < 0.30:
            await message.channel.send(random.choice(horde_lines))
        else:
            response = get_keyword_response(content)
            if response:
                await message.channel.send(response)
            elif "게부라" in content and random.random() < 0.18:
    ...
    # 결투장 대화
    elif is_duel_channel(message.channel):
        if "게부라" in content and random.random() < 0.12:
            await message.channel.send(random.choice(duel_channel_call_lines))
...
async def help_command(ctx):
    text = (
        "**게부라 명령어 목록**\n"
        f"`!결투 @유저` - 다른 유저에게 결투 신청 ({DUEL_CHANNEL_NAME} 전용)\n"
        f"`!수락` - 받은 결투 수락 ({DUEL_CHANNEL_NAME} 전용)\n"
        f"`!거절` - 받은 결투 거절 ({DUEL_CHANNEL_NAME} 전용)\n"
        f"`!게부라결투` - 게부라와 보스전 ({DUEL_CHANNEL_NAME} 전용)\n"
        "`!전적 [@유저]` - 전적 확인\n"
        "`!랭킹` - 서버 결투 랭킹\n"
        "`!스탯 [@유저]` - 스탯 / 호감 확인\n"
        "`!일일` - 오늘의 일일 퀘스트 확인\n"
...
async def duel(ctx, target: discord.Member):
    if not is_duel_channel(ctx.channel):
        await ctx.send(f"결투는 **{DUEL_CHANNEL_NAME}**에서만 한다.")
        return
...
async def accept_duel(ctx):
    if not is_duel_channel(ctx.channel):
        await ctx.send(f"결투 수락은 **{DUEL_CHANNEL_NAME}**에서만 한다.")
        return
...
async def decline_duel(ctx):
    if not is_duel_channel(ctx.channel):
        await ctx.send(f"결투 거절은 **{DUEL_CHANNEL_NAME}**에서만 한다.")
        return
...
async def gebura_duel(ctx):
    if not is_duel_channel(ctx.channel):
        await ctx.send(f"나와 싸우고 싶다면 **{DUEL_CHANNEL_NAME}**으로 와라.")
        return
